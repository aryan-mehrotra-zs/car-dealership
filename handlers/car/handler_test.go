...
func TestHandler_Create(t *testing.T) {
	// todo they are running only when executing alone
	var (
		body = []byte(`{"id":"8f443772-132b-4ae5-9f8f-9960649b3fb4","model":"x","yearOfManufacture":2020,"brand":"BMW","fuelType":2,
	...
	cases := []struct {
		...
		{"success case", body, &car, http.StatusCreated},
		{"entity already exists", body, nil, http.StatusOK},
		{"unmarshal error", []byte(`invalid body`), nil, http.StatusBadRequest},
		{"database error", body, nil, http.StatusInternalServerError},
	}

	for i, tc := range cases {
		h, mockService, r, w := initializeTest(t, http.MethodPost, bytes.NewReader(tc.body), nil, nil)

		mockService.EXPECT().Create(&car).Return(&car, nil)
		mockService.EXPECT().Create(&car).Return(nil, errors.EntityAlreadyExists{})
		mockService.EXPECT().Create(&car).Return(nil, nil)
		mockService.EXPECT().Create(&car).Return(nil, errors.DB{})
...
//
//		mockService.EXPECT().GetAll(tc.filter).Return(tc.output).AnyTimes()
//
//		h.GetAll(w, r)
//
//		resp := w.Result()
